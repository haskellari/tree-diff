...
-- ((,) 1 +(Just 2) -Nothing)
--
-- >>> data Foo = Foo { fooInt :: Int, fooBool :: [Bool] } deriving (Eq, Generic)
-- >>> instance ToExpr Foo
--
-- >>> ppEditExpr prettyP $ ediff (Foo 2 [True]) (Foo 3 [True])
-- Foo {fooBool = [True], fooInt = +2 -3}
--
-- >>> ppEditExpr prettyP $ ediff (Foo 42 [True, False]) (Foo 42 [False, False, True])
-- Foo {fooBool = [+True -False False +True], fooInt = 42}
--
